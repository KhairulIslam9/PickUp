...
BEGIN
	...
	FROM Reservation R
	WHERE R.UserId = @UserId
	AND CONVERT(varchar, EndHour, 8) < CONVERT(varchar,GETDATE(), 8) 
...
Begin
	...
	R.NumPlaceAvailable-R.NumPlaceReserved as NumPlaceAvailable, U.[Url]
...
BEGIN
	SELECT R.ReservationId, C.CustomerId, C.LastName, C.FirstName, C.Email, C.PhoneNum,
		  cast(R.ResDate as nvarchar(50)) as ResDate, R.StartHour, R.EndHour, PR.NumPerson, PR.InputDate
